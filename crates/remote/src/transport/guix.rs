...
impl GuixContainerConnection {
    ...
    fn base_guix_args(&self, working_directory: PathBuf) -> Vec<String> {
        let mut args = vec![
            "shell".to_string(),
            "--container".to_string(),
            "--no-cwd".to_string(),
            ...
            "zed:remote".to_string(),
            format!("--cwd={}", working_directory.to_string_lossy()),
        ];
    ...
    fn command_inside_container(
        ...
        let mut command = self.base_guix_args(working_directory);
...
impl RemoteConnection for GuixContainerConnection {
    ...
    fn build_command(
        ...
        let cwd = self.current_dir_for_command(working_dir);
        Ok(CommandTemplate {
            program: "guix".to_string(),
            args: self.command_inside_container(program, args, env, cwd.clone()),
            env: HashMap::default(),
            cwd: Some(cwd),
        })
    }
...
mod tests {
    ...
    #[test]
    fn test_build_command_without_program_opens_bash_shell() {
        let connection = test_connection(GuixShellOptions::default());
        let command = connection
            .build_command(
                None,
